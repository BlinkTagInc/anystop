...
public class GeoUtils {
    ...
    public static double distanceKm(double lat1, double lon1, double lat2, double lon2) {
        ...
        /*double step1 = Math.sin(deltaLatRad/2)*Math.sin(deltaLatRad/2) + Math.sin(deltaLonRad/2)*Math.sin(deltaLonRad/2)*Math.cos(lat1Rad)*Math.cos(lat2Rad);
        double step2 = 2 * Math.atan2(Math.sqrt(step1), Math.sqrt(1-step1));
        return(EARTH_RADIUS_KM * step2);*/
    ...
    public static boolean geoEquals(GeoPoint one, GeoPoint two) {
    	if(one==null || two==null) {
    		return false;
    	}
    	if (one.getLatitudeE6() == two.getLatitudeE6() && 
    			one.getLongitudeE6() == two.getLongitudeE6()) {
    		return true;
    	}
    	return false;
    }
